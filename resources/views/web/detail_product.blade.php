        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-sm-6 product-details">
                    <h4>{{ $product->name ?? '' }}</h4>
                    <div class="price">
                        <span class="price-was">${{ number_format($product->old_price) ?? 0}}</span>
                        ${{ number_format($product->price) ?? 0}}
                    </div>
                    <div style="padding-top: 5px; padding-bottom: 5px;">
                        <b>Category: </b>{{ $product->category->name ?? '' }}</div>
                    <div style="padding-top: 5px; padding-bottom: 5px;"><b>Brand: </b>{{ $product->brand->name ?? '' }}
                    </div>
                    <form action="{{ route('cart.create') }}" method="post">
                        ...
                        <table class="shop-item-selections">
                            ...
                            <tr>
                                <td><b>Quantity:</b></td>
                                <td>
                                    <input id="amount" name="quantity" type="number"
                                           class="form-control input-sm input-micro" value="1">
                                </td>
                            </tr>
                            <!-- Add to Cart Button -->
                            <tr>
                                <td>&nbsp;</td>
                                <td>
                                    <button class="btn btn-small add-cart"><i class="fa fa-shopping-cart"></i> Add to
                                        cart
                                    </button>
